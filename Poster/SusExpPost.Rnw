...
\begin{itemize}
			...
			\item[] $\qquad\qquad \logit(\blue{\itbf{p}}) = \textbf{log}(School.Pop) + \bfX\bgamma + \bfV\balpha$
			...
			\item[] $\qquad\qquad\logit(\blue{\itbf{q}}) = \textbf{log}(School.Pop) + \bfX\bdelta$
			\end{itemize}
\end{block}
%\vfill
...
\textbf {Elementary School Crime Frequency Coefficients}
...
\textbf {High School Crime Frequency Coefficients}
